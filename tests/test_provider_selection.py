...
def test_select_provider(mock_session, mock_pyperclip):
    ...
    # exact match for openai
    session_instance.prompt.return_value = "openai"
    assert select_provider("text completion") == "openai"
    mock_pyperclip.copy.assert_called_with("openai")

    # fuzzy match
    session_instance.prompt.return_value = "anth"
    assert select_provider("text completion") == "anthropic"
    mock_pyperclip.copy.assert_called_with("anthropic")

    # no match
    session_instance.prompt.return_value = "invalid"
    assert select_provider("text completion") is None
